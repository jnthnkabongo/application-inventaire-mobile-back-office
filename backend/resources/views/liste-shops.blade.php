    ...
    <div class="container my-5">
        <div class="row">
            ...
            <div class="col-lg-3 col-md-4 col-sm-6">
                <a class="btn btn-primary" href=""><i class="bi bi-shop me-2"></i> Ajouter une région</a>
            </div>
        </div>
        ...
        <div class="table-responsive shadow-sm rounded-4 bg-white">
            <table class="table table-striped table-hover align-middle mb-0">
                ...
                <tbody>
                    @forelse ($liste_shops as $shops)
                        <tr>
                            <td>{{ ($liste_shops->perPage() * ($liste_shops->currentPage() - 1 ))+ $loop->iteration }}</td>
                            <td>{{ Str::ucfirst($shops->Regions->name)}}</td>
                            <td>{{ Str::ucfirst($shops->name) }}</td>
        ...
        <nav class="app-pagination">
            <ul class="pagination justify-content-center mt-2 pb-2">
                <li class="page-item disabled">
                    {{ $liste_shops->withQueryString()->links() }}
                </li>
            </ul>
        </nav>
    </div>
